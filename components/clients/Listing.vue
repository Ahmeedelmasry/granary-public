      ...
      <v-row v-if="clients" class="px-4">
        ...
        >
          ...
          <v-data-table-server
            ...
            v-model:page="page"
            :items-length="clients.totalElements ? clients.totalElements : 0"
              ...
              <v-pagination
                ...
                :length="
                  clients.totalElements
                    ? Math.ceil(clients.totalElements / perPage)
                    : 1
                "
              ></v-pagination
            ></v-col>
...
const openDel = (item) => {
  mainModule.handleErr(
    "alert",
    "حذف توريد",
    `يرجي تاكيد الطلب قبل الحذف, تريد تاكيد حذف العميل ${item.name} ؟`,
    { ...item, url: "client" }
  );
