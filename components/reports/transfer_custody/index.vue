    ...
    <v-container class="page_container" fluid>
      <v-row class="px-4 mt-1">
        ...
        <v-col
          ...
          id="printable"
        >
          <h1 dir="rtl" class="my-2 hide_till_print">تقرير تحويلات العهدة</h1>
          <v-data-table-server
            :headers="headers"
            :items="transfers.content"
            :loading="loading"
          ...
          >
            ...
            <template v-slot:item="{ item }">
              <tr>
                <td>{{ item.index + 1 }}</td>
                <td>
                  {{ item.selectable.adminUser.userName }}
                </td>
                <td>
                  {{ item.selectable.custodyUser.userName }}
                </td>
                <td>
                  {{ item.selectable.previousCustody }}
                </td>
                <td>
                  {{ item.selectable.addedValue }}
                </td>
                <td>
                  {{ parseInt(item.selectable.previousCustody) +  parseInt(item.selectable.addedValue)}}
                </td>
                <td>
...
const headers = ref([
  { title: "رقم", key: "index" },
  { title: "المرسل", key: "adminUser" },
  { title: "المستلم", key: "custodyUser" },
...
// Props
const props = defineProps(["transfers", "loading"]);
